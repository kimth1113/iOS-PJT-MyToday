...
//    }()
    ...
    override func configureUI() {
        ...
        [backgroundImage, dateButton, leftButton, rightButton, analysisMonthView, analysisRandomView, analysisEmoticonView].forEach {
            addSubview($0)
        }
    }
    ...
    override func setConstraint() {
        ...
        analysisMonthView.snp.makeConstraints { make in
            ...
            make.trailing.equalTo(analysisRandomView.snp.leading).offset(-12)
            make.width.equalTo(analysisRandomView)
            make.height.equalTo(analysisMonthView.snp.width)
        }
        
        analysisRandomView.snp.makeConstraints { make in
            make.top.equalTo(dateButton.snp.bottom).offset(12)
            make.trailing.equalTo(self).offset(-12)
            make.height.equalTo(analysisRandomView.snp.width)
        }
        
        analysisEmoticonView.snp.makeConstraints { make in
            make.top.equalTo(analysisRandomView.snp.bottom).offset(12)
            make.leading.trailing.equalTo(self).inset(12)
            make.height.equalTo(analysisEmoticonView.snp.width).multipliedBy(0.9)
        }
    }
}
